import { toNumber } from "@/lib/dashboard/read-models/aggregate-queries"
import { getGymLocalMonthWindow } from "@/lib/domain/date-boundaries"
import { toCsv, toDateOnly } from "./csv.ts"
import { getRevenueTrendForTimeZone } from "@/lib/dashboard/read-models/subscription-aggregates"
...
export async function getMonthlyReportCsv({
  ...
  const [
    ...
    client.membershipPayment.count({
      where: {
        gymId: gym.id,
        OR: [{ status: "OVERDUE" }, { status: "PENDING", dueAt: { lt: end } }],
      },
    }),
